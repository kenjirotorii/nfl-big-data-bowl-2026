...
class _RPEAttention(MessagePassing):
    def __init__(
        self,
        hidden_dim: int,
        num_heads: int,
    ...
    def forward(
        ...
        x = target
        x = x + self.attn_postnorm(self._attn_block(x_src, x_tgt, r, edge_index))
        x = x + self.ff_postnorm(self._ff_block(self.ff_prenorm(target)))
        return x

    def message(
        self, q_i: Tensor, k_j: Tensor, v_j: Tensor, r: Tensor, index: Tensor, ptr: Any
    ) -> Tensor:
        # Note: TLDR; it's a software issue, don't need to read.
        #
        # The correct type for "ptr" is Tensor | None. However,
        # 1. PyG (PyTorch Geometric) requires the annotation to be Optional[Tensor] and
        # does not accept the modern union syntax using "|".
        # 2. As of February 13, 2025, VS Code automatically reformats Optional[...] to
        # "... | None" and offers no setting to disable this
        # conversion without turning off the Python language server.
        # Simply annotate the type as Any.
        k_j += self.to_k_r(r).view(-1, self.num_heads, self.head_dim)
        v_j += self.to_v_r(r).view(-1, self.num_heads, self.head_dim)
    ...
    def _attn_block(
        self, x_src: Tensor, x_dst: Tensor, r: Tensor, edge_index: Tensor
    ) -> Tensor:
...
class RPESelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, r: Tensor, edge_index: Tensor) -> Tensor:
        return self._attn(x, x, r, edge_index)


class RPECrossAttention(nn.Module):
    def __init__(
        self,
        hidden_dim: int,
        num_heads: int,
    ...
    def forward(
        self, source: Tensor, target: Tensor, r: Tensor, edge_index: Tensor
    ) -> Tensor:
        return self._attn(source, target, r, edge_index)
